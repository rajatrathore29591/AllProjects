...
namespace TradeMark
{
    public partial class BulkSearchResult : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                //Get Request id and mark list
                DataTable MarkSearchRecord = oMultiSearchService.GetAllMarkToSearch();
                foreach (DataRow dtRow in MarkSearchRecord.Rows)
                {
                    var sqlquery = "select * from MarkToBeSearch where RequestId = " + dtRow[0];
                    DataTable dt = oMultiSearchService.GetRecordByQuery(sqlquery);

                    // On all tables' columns
                    foreach (DataRow dr in dt.Rows)
                    {
                        var query = "select (convert(varchar,ClassNo)+','+convert(varchar,USClassDescriptionId)) as UsClassno, USClassDescription from USClassDescriptions where USClassDescription = '" + dr[5] + "'";
                        DataTable dtUsClass = oMultiSearchService.GetRecordByQuery(query);
                        if (dtUsClass.Rows.Count > 0)
                        {
                            if (dr["Mark"] != null && dr["GoodsService"] != null && dr["Component"] != null && dr["LongForm"] == null)
                            {
                                radioValue = "1";
                            }
                            else if (dr["Mark"] != null && dr["GoodsService"] != null && dr["Component"] == null && dr["LongForm"] == null)
                            {
                                radioValue = "2";
                            }
                            else if (dr["Mark"] != null && dr["GoodsService"] != null && dr["Component"] == null && dr["LongForm"] != null)
                            ...
                            TradeMarkSearch(dr["Mark"].ToString(), dtUsClass.Rows[0]["UsClassno"].ToString(), dr["Component"].ToString(), dr["LongForm"].ToString(), radioValue);
                        }
                    }
                }
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow dtRow in MarkSearchRecord.Rows)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    var query = "select (convert(varchar,ClassNo)+','+convert(varchar,USClassDescriptionId)) as UsClassno, USClassDescription from USClassDescriptions where USClassDescription = " + dr[1].ToString();
                    DataTable dtUsClass = oMultiSearchService.GetRecordByQuery(query);

                    TradeMarkSearch(dr["Mark"].ToString(), dtUsClass.Rows[0]["UsClassno"].ToString(), dr["Component"].ToString(), dr["LongForm"].ToString(), "1");
                }
            }

            //Generate condition for option (hdnRdoOption.Value, 1,2,3,4)
        ...
        public void TradeMarkSearch(string trademarkText, string goodsServicesId, string ComponentFullFormText, string FullFormText, string searchOption)
        {
            ...
            string selectedUsClassDescription = oService.GetUsclassdescription(uSClassDescriptionId);

            //Remove Services here
            if (Convert.ToInt16(usClassId) >= 100 && Convert.ToInt16(usClassId) <= 107)
            {
                //hdnGoodsName.Value = hdnGoodsName.Value.ToLower().Replace("services", "").Trim();
                selectedUsClassDescription = selectedUsClassDescription.ToLower().Replace("services", "").Trim();
            }
            //End

            string[] arrayStringSAMEGS = selectedUsClassDescription.Split(' ');
            ...
            if (!string.IsNullOrEmpty(trademarkText))
            {
                ...
                //Call query execute method to get trade marks details
                ...
                if (dtResult != null && dtResultDistinct > 0)
                {
                    ...
                    htmlResultFinal = htmlResultFinal + "<th style='background-color:#eaeaea;font-size:14px; padding: 5px; text-align: center; vertical-align: middle; color:#000;'>Owner</th>";
                    htmlResultFinal = htmlResultFinal + "<th style='background-color:#eaeaea;font-size:14px; padding: 5px; text-align: center; vertical-align: middle; color:#000;'>Goods or Services Description</th>";
                ...
                if ((dtResult == null || dtResult.Rows.Count == 0) && (searchOption == "2" || searchOption == "3" || searchOption == "4"))
                {
                    ...
                    if (searchOption == "2" || searchOption == "4")
                    {
                        ...
                        if (!string.IsNullOrEmpty(ComponentFullFormText))
                        {
                            ...
                            var arryComponents = ComponentFullFormText.Split(',');
                            ...
                            for (int component = 0; component < arryComponents.Length; component++)
                            {
                                ...
                                sqlQuery += "select Top 10 SerialNo, Registrationno,Status,MarkIdentification,OwnerName,Casefilestatementstext from TradeMark where " + seachWhereQuery + " Order by Registrationno desc, SerialNo desc ";
                                sqlQuery += " ";
                        ...
                        if (dtResultdataSet != null && dtResultdataSet.Tables.Count > 0)
                        {
                            ...
                            var arryComponentsNew = ComponentFullFormText.Split(',');
                ...
                //VD
                var userCredits = oUserService.GetUserCreditsById(Session["UserId"].ToString());
                litScript.Text = "<script>showCredits(" + userCredits + ");</script>";
            }
                ...
                if (dtResultLevel3 != null && dtResultLevel3.Tables.Count > 0)
                {
                    ...
                    for (int full = 0; full < arryFullForm.Length; full++)
                    {
                        ...
                        if (gridColor == ScoringColoringModel.Red)
                        {
                            htmlResultFinal = string.Empty;
                            htmlResultFinal = "<div class='table - responsive'> <table class='table table-bordered table - hover'><thead><tr>";
                            htmlResultFinal = htmlResultFinal + "<th class='header'>Serial Number</th> <th class='header'>Registration Number</th><th class='header'>Word Mark</th>";
                            htmlResultFinal = htmlResultFinal + "</tr></thead><tbody>";
        ...
        protected void GetNear5ConditionStringForAssociatedUsClass(int usClassDescriptionId)
        {
            ...
            if (dtAssociatedUSClassDescription != null && dtAssociatedUSClassDescription.Rows.Count > 0)
            {
                foreach (DataRow dr in dtAssociatedUSClassDescription.Rows)
                {
                    ...
                    if (Convert.ToInt16(dr["ClassNo"].ToString()) >= 100 && Convert.ToInt16(dr["ClassNo"].ToString()) <= 107)
                    {
                        usDescription = usDescription.ToLower().Replace("services", "").Trim();
                    }
                    //End
            ...
            if (dtResult != null && dtResult.Rows.Count > 0)
            {
                ...
                if (cautionColor == ScoringColoringModel.Yellow)
                {
                    foreach (DataRow dr in dtResult.Rows)
                    {
                        ...
                        htmlResult = htmlResult + "<tr><td class='searh-td-right' style='padding: 15px; margin-bottom: 20px; border: 1px solid transparent; border-radius: 4px; color: #8a6d3b;background-color: #fff1a7;border-color: #faebcc;'> <a href='TradeMarkDetailPage.aspx?Sn=" + dr["SerialNo"].ToString() + "&Rn=" + dr["Registrationno"].ToString() + "'>" + dr["SerialNo"].ToString() + "</a></td>";
                        ...
                        htmlResult = htmlResult + "<td class='searh-td-right' style='padding: 15px; margin-bottom: 20px; border: 1px solid transparent; border-radius: 4px; color: #8a6d3b;background-color: #fff1a7;border-color: #faebcc;'> " + dr["Registrationno"].ToString() + "</td>";
                        ...
                        htmlResult = htmlResult + "<td style='padding: 15px; margin-bottom: 20px; border: 1px solid transparent; border-radius: 4px; color: #8a6d3b;background-color: #fff1a7;border-color: #faebcc;'>" + dr["MarkIdentification"].ToString() + "</td>";
                        htmlResult = htmlResult + "<td style='padding: 15px; margin-bottom: 20px; border: 1px solid transparent; border-radius: 4px; color: #8a6d3b;background-color: #fff1a7;border-color: #faebcc;'>" + dr["OwnerName"].ToString() + "</td>";
                        htmlResult = htmlResult + "<td style='padding: 15px; margin-bottom: 20px; border: 1px solid transparent; border-radius: 4px; color: #8a6d3b;background-color: #fff1a7;border-color: #faebcc;'>" + dr["Casefilestatementstext"].ToString() + "</td>";
                ...
                else
                {
                    foreach (DataRow dr in dtResult.Rows)
                    {
                        ...
                        htmlResult = htmlResult + "<td style='color:" + cautionColor + ";'>" + dr["OwnerName"].ToString() + "</td>";
                        htmlResult = htmlResult + "<td style='color:" + cautionColor + ";'>" + dr["Casefilestatementstext"].ToString() + "</td>";
        ...
        protected void btnSavePdf_Click(object sender, EventArgs e)
        {
            ...
            if ((DataTable)Session["Datatable"] != null)
            {
                ...
                DataRow[] drFilterRed = objDataTable.Select("CellColor like '%Red%'");
                {
                    ...
                    else
                    {
                        ...
                        if (drFilterYellow.Count() > 0)
                        {
                            ...
                            color = FontFactory.GetFont(ScoringColoringModel.pdfFontSet, 12, new BaseColor(System.Drawing.ColorTranslator.FromHtml("#ffa800").ToArgb()));
                ...
                PdfPTable dataTableCellHeaderTable = new PdfPTable(5);
                dataTableCellHeaderTable.HorizontalAlignment = 0;

                float[] widths = new float[] { 2f, 2f, 5f, 2f, 5f };
                ...
                PdfPCell cellOwnerMark = new PdfPCell(new Phrase(ScoringColoringModel.pdfOwnerCellHeading, tableHeaderCellFont)) { Border = PdfPCell.LEFT_BORDER };
                cellOwnerMark.PaddingTop = 7.5f;
                cellOwnerMark.PaddingBottom = 7.5f;
                cellOwnerMark.BackgroundColor = new BaseColor(System.Drawing.ColorTranslator.FromHtml("#D3D3D3").ToArgb());
                cellOwnerMark.BorderColor = BaseColor.WHITE;
                dataTableCellHeaderTable.AddCell(cellOwnerMark);

                PdfPCell cellGoodsServices = new PdfPCell(new Phrase(ScoringColoringModel.pdfGoodsServicesCellHeading, tableHeaderCellFont)) { Border = PdfPCell.LEFT_BORDER };
                cellGoodsServices.PaddingTop = 7.5f;
                cellGoodsServices.PaddingBottom = 7.5f;
                cellGoodsServices.BackgroundColor = new BaseColor(System.Drawing.ColorTranslator.FromHtml("#D3D3D3").ToArgb());
                cellGoodsServices.BorderColor = BaseColor.WHITE;
                dataTableCellHeaderTable.AddCell(cellGoodsServices);
                ...
                {
                    if (dr.ItemArray[6].ToString() == "Yellow")
                    {
                    ...
                    {
                        cellColor = FontFactory.GetFont(ScoringColoringModel.pdfFontSet, 12, new BaseColor(System.Drawing.ColorTranslator.FromHtml(dr.ItemArray[6].ToString()).ToArgb())); ;
                    }
                    ...
                    dataTableCellHeaderTable.AddCell(new PdfPCell(new Phrase(dr.ItemArray[4].ToString(), cellColor)) { PaddingBottom = 5, Border = 1, PaddingTop = 5 });
                    dataTableCellHeaderTable.AddCell(new PdfPCell(new Phrase(dr.ItemArray[5].ToString(), cellColor)) { PaddingBottom = 5, Border = 1, PaddingTop = 5 });
                }

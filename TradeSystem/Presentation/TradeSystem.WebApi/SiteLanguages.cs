...
namespace TradeSystem.MVCWeb
{
    public class SiteLanguages
    {
        public static List<Languages> AvailableLanguages = new List<Languages>
        {
             ...
             new Languages{ LangFullName = "ES", LangCultureName = "es"},
             //new Languages{ LangFullName = "বাংলা", LangCultureName = "bn"}
        };

        public static bool IsLanguageAvailable(string lang)
        {
            bool bl = AvailableLanguages.Where(a => a.LangCultureName.Equals(lang)).FirstOrDefault() != null ? true : false;
            return bl;
        }

        public static string GetDefaultLanguage()
        {
            var langCulture = Thread.CurrentThread.CurrentCulture;
            if (langCulture.Name == "es-ES")
            {
